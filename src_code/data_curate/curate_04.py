...
def curate_rolling_player_stats(config, curr_date, first_days, days):
        ...
        for index, row in team_df.iterrows():
            ...
            if len(prior_game_dates) >= days:
                ...
                for player_id in player_id_list:
                    ...
                    p_avg_gameToi.append(sum(player_prior_games_df['gameToi']) / games_played)
                    ...
                    p_avg_gamePoints.append(sum(player_prior_games_df['gamePoints']) / games_played)
                    p_avg_anyPoints.append(sum(player_prior_games_df['anyPoints']) / games_played)
                    p_avg_gameShots.append(sum(player_prior_games_df['gameShots']) / games_played)
    ...
    playerFeature_df = pd.DataFrame({
        ...
        f"avg_gamePoints_{days}": p_avg_gamePoints,
        f"avg_anyPoints_{days}": p_avg_anyPoints,
        f"p_avg_gameShots_{days}": p_avg_gameShots,
        f"p_avg_anyShots_01p_{days}": p_avg_anyShots_01p,
        f"p_avg_anyShots_02p_{days}": p_avg_anyShots_02p,
        f"p_avg_anyShots_03p_{days}": p_avg_anyShots_03p,
        f"p_avg_anyShots_04p_{days}": p_avg_anyShots_04p,
    })

    playerFeature_df.to_csv(f"storage/playerFeature_{days}.csv", index=False)
...
def curate_proj_player_data(config, curr_date, first_days, days):
    ...
    if first_days:
        result_df = pd.merge(playerFeature_df, basic_df, how='left', on=['team', 'gameDate'])
